...
class TuneModels:
    ...
    def tune_logistic(self, params):
        ...
        y_pred = clf.predict_proba(self.X_train0)[:, 1]
        precisions, recalls, thresholds = precision_recall_curve(self.y_train0, y_pred)
    ...
    def tune_xgboost(self, params):
        ...
        y_pred = xgb.predict_proba(self.X_train0)[:, 1]
        precisions, recalls, thresholds = precision_recall_curve(self.y_train0, y_pred)
        thrs, _ = better_threshold(precisions, recalls, thresholds)
    ...
    def tune_trees(self, params):
        ...
        y_pred = rf.predict_proba(self.X_train0)[:, 1]
        precisions, recalls, thresholds = precision_recall_curve(self.y_train0, y_pred)
        thrs, _ = better_threshold(precisions, recalls, thresholds)
        ...
        p = (p >= thrs).astype(int)
        _, metric = evaluate(p, self.y_valid, plot_matrix=False)
    ...
    def tune_knn(self, params):
        ...
        y_pred = neigh.predict_proba(self.X_train0)[:, 1]
        precisions, recalls, thresholds = precision_recall_curve(self.y_train0, y_pred)
        thrs, _ = better_threshold(precisions, recalls, thresholds)
    ...
    def tune_nn_dropout(self, params):
        ...
        with tf.device(get_available_gpus()[0]):
            model.fit(self.X_train0, 
                        ...
                        verbose=1)
        p = model.predict(self.X_train1)
        y_pred = model.predict(self.X_train0)
        precisions, recalls, thresholds = precision_recall_curve(self.y_train0, y_pred)
        thrs, _ = better_threshold(precisions, recalls, thresholds)
        y_pred = (p >= thrs).astype(int)
...
class ModelFeaturesGroups:
    ...
    def LGBM_1_group(self, params):
        ...
        for key, value in self.groups.items():
            ...
            X_train0, X_train1, y_train0, _ = train_test_split(train, y_train, test_size=0.4, random_state=42)
...
class ModelRowGroups:
    ...
    def XGboost(self):
        ...
        list_ns = [n, n*2, n*3, n*4]

        buckets_Xtr0 = np.digitize(pd.DataFrame(self.X_train0.sum(axis=1)).to_numpy(), bins=list_ns).reshape(1, -1)
        buckets_Xtr1 = np.digitize(pd.DataFrame(self.X_train1.sum(axis=1)).to_numpy(), bins=list_ns).reshape(1, -1)
        buckets_valid = np.digitize(pd.DataFrame(self.valid.sum(axis=1)).to_numpy(), bins=list_ns).reshape(1, -1)
        buckets_test = np.digitize(pd.DataFrame(self.test.sum(axis=1)).to_numpy(), bins=list_ns).reshape(1, -1)
        ...
        for bucket in np.unique(buckets_Xtr0):
            ...
            Xtr0 = self.X_train0[(buckets_Xtr0 == bucket)[0], :]
            ytr0 = self.y_train0[(buckets_Xtr0 == bucket)[0]]


            mdl = XGBClassifier(n_jobs=-1, random_state=42)
            mdl.fit(Xtr0, ytr0)

            Xtr1 = self.X_train1[(buckets_Xtr1 == bucket)[0], :]
            p_tr1[(buckets_Xtr1 == bucket)[0]] = mdl.predict_proba(Xtr1)[:, 1]

            Xval1 = self.valid[(buckets_valid == bucket)[0]]
            p_vl1[(buckets_valid == bucket)[0]] = mdl.predict_proba(Xval1)[:, 1]

            Xts = self.test[(buckets_test == bucket)[0]]
            p_ts[(buckets_test == bucket)[0]] = mdl.predict_proba(Xts)[:, 1]
    ...
    def LGBM(self):
        ...
        buckets_Xtr0 = np.digitize(pd.DataFrame(self.X_train0.sum(axis=1)).to_numpy(), bins=list_ns)
        buckets_Xtr1 = np.digitize(pd.DataFrame(self.X_train1.sum(axis=1)).to_numpy(), bins=list_ns)
        buckets_valid = np.digitize(pd.DataFrame(self.valid.sum(axis=1)).to_numpy(), bins=list_ns)
        buckets_test = np.digitize(pd.DataFrame(self.test.sum(axis=1)).to_numpy(), bins=list_ns)
        ...
        for bucket in np.unique(buckets_Xtr0):
            ...
            Xtr0 = self.X_train0[buckets_Xtr0 == bucket]
            ytr0 = self.y_train0[buckets_Xtr0 == bucket]


            mdl = LogisticRegression(random_state=42)
            mdl.fit(Xtr0, ytr0)

            Xtr1 = self.X_train1[buckets_Xtr1 == bucket]
            p_tr1[buckets_Xtr1 == bucket] = mdl.predict_proba(Xtr1)[:, 1]

            Xval1 = self.valid[buckets_valid == bucket]
            p_vl1[buckets_valid == bucket] = mdl.predict_proba(Xval1)[:, 1]

            Xts = self.test[buckets_test == bucket]
            p_ts[buckets_test == bucket] = mdl.predict_proba(Xts)[:, 1]
...
class FeatureImportance:
    ...
    def result(self, X, y):
        ...
        print("Random Column")
        X_random = pd.concat([X, pd.DataFrame({'random':[np.random.uniform(0.0, 100.0) for i in range(X.shape[0])]})], axis=1)
